...
# Tile PSF and crop kernel tile to match encoder block dims
tiled_psf = Lambda(tile_and_crop_kernel, output_shape=(96,96,1))(i_ker)

# Early fusion
x0 = Concatenate()([i_blur, tiled_psf])  # (96,96,2)
...
class WeightedLossModel(Model):
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            weighted_mse = tf.reduce_mean(tf.square(y_pred - y_true)* (weight_map*10+0.02) )
            ellip_loss = ellipticity_loss(y_true, y_pred, weight_map)

            total_loss = 50*weighted_mse + 0.005* ellip_loss  # weight the ellipticity term
    ...
    def test_step(self, data):
        ...
        weighted_mse = tf.reduce_mean(tf.square(y_pred - y_true)* (weight_map*10 +0.02) )
        ellip_loss = ellipticity_loss(y_true, y_pred, weight_map)
        total_loss = 50*weighted_mse + 0.005 * ellip_loss
...
# Save model
model.save('/scratch/bell/dutta26/psf_datasets/unet_psf_model.h5')
